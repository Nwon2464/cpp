...
int main() {
  ...
  f0r(k, 3) {
    ...
    while (!dq.empty()) {
      ...
      f0r(i, 4) {
        ...
        if (v) dq.pb(mp(mp(nr, nc), x.s + 1));
        else dq.push_front(mp(mp(nr, nc), x.s));
      }
    }
    // f0r(i, n) {
  ...
  f0r(i, n) f0r(j, m) {
    if (mat[i][j] != '#') {
      if (mat[i][j] == '.') mv = min(mv, (ll)dist[0][i][j] + dist[1][i][j] + dist[2][i][j] - 2);
      else mv = min(mv, (ll)dist[0][i][j] + dist[1][i][j] + dist[2][i][j]);
    }
  }
